...
#include <list>
#include <algorithm>
#include <iterator> // fot std::advance
...
void test3() {
    ...
    persons.resize(5); // use the Person default constructor
    display(persons);
}
...
void test4() {
    ...
    std::advance(it, -4); // move back 4 and point to 100
    std::cout << *it << std::endl;

    l.erase(it); // it becomes invalid
    display(l);
}
...
void test6() {
    ...
    //         [](const Person &p1, const Person &p2) {return p1.age < p2.age; }); // wont work because sort needs random access of the storage
    stooges.sort();
