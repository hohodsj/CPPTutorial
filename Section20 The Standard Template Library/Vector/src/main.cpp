...
void test2() {
    ...
    vec.shrink_to_fit(); // C++11 shrink the amount of storage allocated to the exact size
    display(vec);
    std::cout << "\nvec size: " << vec.size() << std::endl;
    std::cout << "vec max size: " << vec.max_size() << std::endl;
    std::cout << "vec capacity: " << vec.capacity() << std::endl;
...
void test10() {
    ...
    // Note you dont need vec2.end because it is limit by vec1
