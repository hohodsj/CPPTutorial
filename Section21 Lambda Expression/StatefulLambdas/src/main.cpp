...
void test1() {
    ...
    int local_x {100};
    // local_x = 200;
    auto l = [local_x]() { // capture at this point
...
void test2() {
    ...
    int x {100};
    auto l = [x] () mutable { // calls constructor
        x += 100;
        std::cout << x << std::endl;
    };

    l(); // only operator will be called
    std::cout << x << std::endl;
...
void test4() {
    ...
    int x {100};
    int y {200};
    int z {300}; // Note that z is not vaptured in the lambda since it is not used.
...
// lambda body, all three of them will be captured by reference.
void test5() {
	std::cout << "\n---Test5 --------------------------" << std::endl;
...
// [=], [&], and [this] are eqivalent when captureing an object's member variables - all are captured by reference
...
void test8() {
    ...
    // Default capture [=]
    // This has been deprecated by C++20
...
public:
    ...
    // people whos age > max_age AND it limits the number of
